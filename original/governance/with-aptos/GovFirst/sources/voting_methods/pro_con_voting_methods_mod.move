module gov_first::pro_con_voting_methods_mod {
  use std::string;
  use gov_first::voting_mod;

  struct ProConVotingMethods has store {
    yes_votes: u64,
    no_votes: u64,
  }
  ...
  public fun propose(
    ...
    let methods = ProConVotingMethods { yes_votes: 0, no_votes: 0 };
    voting_mod::propose<ProConVotingMethods>(
      proposer,
      title,
      content,
      methods,
  ...
  fun initialize_for_test(framework: &signer, owner: &signer) {
    ...
    voting_mod::initialize<ProConVotingMethods>(owner);
  }
  ...
  fun test_propose(framework: &signer, owner: &signer, account: &signer) {
    ...
    assert!(voting_mod::exists_proposal<ProConVotingMethods>(1, account_address), 0)
  }
}